...
(function(){
    ...
    function calculateValues(){
        var productSelect = qs('product-select');
        var quantityInput = qs('quantity-sold');
        var expensesInput = qs('total-expenses');
        var totalSalesDisplay = qs('total-sales-display');
        var totalSalesHidden = qs('total-sales');
        var summarySales = qs('summary-sales');
        var summaryExpenses = qs('summary-expenses');
        var summaryRoi = qs('summary-roi');
        var roiValueHidden = qs('roi-value');

        // defensive checks
        if (!productSelect || !quantityInput || !expensesInput || !totalSalesDisplay || !totalSalesHidden) {
            return;
        }

        var price = parseNumber(productSelect.options[productSelect.selectedIndex]?.getAttribute('data-price'));
        var quantity = parseNumber(quantityInput.value);
        var expenses = parseNumber(expensesInput.value);

        var totalSales = price * quantity;
        var roi = totalSales - expenses;

        totalSalesDisplay.textContent = formatCurrency(totalSales);
        totalSalesHidden.value = totalSales.toFixed(2);

        if (summarySales) summarySales.textContent = formatCurrency(totalSales);
        if (summaryExpenses) summaryExpenses.textContent = formatCurrency(expenses);
        if (summaryRoi) {
            summaryRoi.textContent = formatCurrency(roi);
            // small visual tweak: red when negative
            if (roi < 0) {
                summaryRoi.classList.add('text-danger');
            } else {
                summaryRoi.classList.remove('text-danger');
            }
        }

        if (roiValueHidden) roiValueHidden.value = roi.toFixed(2);
    }

    function attachListeners(){
        var ids = ['product-select','quantity-sold','total-expenses'];
        ids.forEach(function(id){
            var el = qs(id);
            if (!el) return;
            var event = (el.tagName === 'SELECT') ? 'change' : 'input';
            el.removeEventListener(event, calculateValues);
            el.addEventListener(event, calculateValues);
        });
    }

    document.addEventListener('DOMContentLoaded', function(){
        // Image preview
        var receiptInput = qs('receipt-images');
        if (receiptInput) {
            receiptInput.addEventListener('change', function(e){
                var previewContainer = qs('preview-container');
                var imagePreview = qs('image-preview');
