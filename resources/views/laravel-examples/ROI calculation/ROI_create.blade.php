                ...
                <div class="row gx-4">
                    <div class="col-auto">
                        <div class="avatar avatar-xl position-relative">
                            <div class="w-100 border-radius-lg shadow-sm bg-gradient-success d-flex align-items-center justify-content-center"
                                style="height: 74px;">
        ...
        <div class="container-fluid py-4">
            <div class="card">
                ...
                <div class="card-body pt-4 p-3">
                    ...
                    <form
                        action="{{ isset($roi) ? url('ROI-management/update/' . $roi->id) : url('ROI-management/store') }}"
                        method="POST" enctype="multipart/form-data" id="roiForm">
                        @csrf
                        @if (isset($roi))
                            @method('PUT')
                        @endif
                        ...
                        <div class="card bg-gradient-light mb-4">
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="product-select"
                                                class="form-control-label">{{ __('Select Product') }} <span
                                            ...
                                            <div class="@error('product_id') border border-danger rounded-3 @enderror">
                                                <select class="form-control" id="product-select" name="product_id"
                                                    onchange="calculateValues()" required>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="quantity-sold" class="form-control-label">{{ __('Quantity Sold') }}
                                                <span class="text-danger">*</span></label>
                                            <div class="@error('quantity_sold') border border-danger rounded-3 @enderror">
                                                <input class="form-control"
                                                    value="{{ old('quantity_sold', $roi->quantity_sold ?? '') }}"
                                                    type="number" min="1" placeholder="Enter quantity sold"
                                                    id="quantity-sold" name="quantity_sold" oninput="calculateValues()"
                                                    required>
                                                @error('quantity_sold')
                                                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                                @enderror
                                ...
                                <div class="row mt-2">
                                    <div class="col-12">
                                        <div class="alert alert-success mb-0" role="alert">
                                            <strong><i class="fas fa-calculator me-2"></i>Total Sales
                                                (Auto-calculated):</strong>
                                            <span class="float-end fs-5" id="total-sales-display">RM 0.00</span>
                                            <input type="hidden" name="total_sales" id="total-sales"
                                                value="{{ old('total_sales', $roi->total_sales ?? '0') }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card bg-gradient-light mb-4">
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="total-expenses"
                                                class="form-control-label">{{ __('Total Expenses (RM)') }} <span
                                                    class="text-danger">*</span></label>
                                            <div class="@error('total_expenses') border border-danger rounded-3 @enderror">
                                                <div class="input-group input-group-lg">
                                                    ...
                                                    <input class="form-control"
                                                        ...
                                                        id="total-expenses" name="total_expenses"
                                                        oninput="calculateValues()" required>
                                                </div>
                                                @error('total_expenses')
                                                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                                @enderror
                                                <small class="text-muted">
                                                    <i class="fas fa-lightbulb"></i> Enter the total amount from your
                                                    receipt(s)
                                                </small>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="record-date" class="form-control-label">{{ __('Record Date') }}
                                                <span class="text-danger">*</span></label>
                                            <div class="@error('record_date') border border-danger rounded-3 @enderror">
                                                <input class="form-control form-control-lg"
                                                    ...
                                                    type="date" id="record-date" name="record_date" required>
                                                @error('record_date')
                                                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                                @enderror
                                            </div>
                            ...
                            <div class="card-body p-4">
                                ...
                                <div class="row text-center">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="hidden" name="roi" id="roi-value"
                                            value="{{ old('roi', $roi->roi ?? '0') }}">
                                    </div>
                                </div>
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button type="submit" class="btn btn-lg bg-gradient-success text-white">
                                <i
                                    class="fas fa-save me-2"></i>{{ isset($roi) ? __('Update Record') : __('Save Record') }}
                            </button>
                        </div>
                    ...
                    if (e.target.files.length > 0) {
                        ...
                        Array.from(e.target.files).forEach((file, index) => {
                            ...
                            reader.onload = function(event) {
                                ...
                                div.innerHTML =
                                    `<img src="${event.target.result}" alt="Receipt ${index + 1}" class="border rounded shadow-sm" style="width: 120px; height: 120px; object-fit: cover;">`;
